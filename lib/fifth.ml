...
let rec simulate program acc =
  ...
  | x :: xs -> (
      ...
      | Pop -> (
          match Stack.pop_opt stack with
          | Some _ -> simulate xs (acc + 1)
          | None -> "Pop operation being performed on empty stack" |> failwith)
      | Dup ->
          (match Stack.top_opt stack with
          | Some v -> Stack.push v stack
          | None -> "Empty stack. Can't do Dup, Dupbass." |> failwith);
          simulate xs (acc + 1)
      | Add ->
          arith_bin_op ( + ) stack;
          simulate xs (acc + 1)
      ...
      | Dump -> (
          ...
          | Some v ->
              ...
              simulate xs (acc + 1)
          | None ->
              "Dump operation can't be performed on an empty stack" |> failwith)
      | Div ->
          (match Stack.pop_opt stack with
          | Some a when a <> 0 -> (
              match Stack.pop_opt stack with
              ...
              | None -> "Second operand isn't present in the stack" |> failwith)
          | None ->
              "Binary operation being performed on empty stack" |> failwith
          | _ -> "Numerator cannot be zero" |> failwith);
          simulate xs (acc + 1)
      | If (then_branch, else_branch) -> (
          match Stack.pop_opt stack with
          | Some v ->
              if v <> 0 then simulate then_branch acc
              else simulate else_branch acc
          | None -> raise_so "If condition on line %d with empty stack" acc)
      | DeFun (name, body) ->
          Hashtbl.add function_table name body;
          simulate xs (acc + 1)
      | CallFun name -> (
